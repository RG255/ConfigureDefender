...
'@
			...
			# Per-item helper called once per event row while the enclosing (already-traced) Get-CDEvent
			# assembles its result - deliberately NOT given its own trace stamp/try-catch, same exemption
			# as DnsBinaryReader's per-element read methods (see the function-trace plan doc).
			...
			# Per-item helper (see Convert-Dev comment above) - exempt from trace/try-catch.
...
'@
	...
	{
		Write-MyCatchAudit -Source 'Get-CDEvent: querying/parsing Defender or CodeIntegrity events' -ErrorRecord $_
		throw
	}
}
