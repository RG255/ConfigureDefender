...
try
{
	...
	{
		Write-TestHeader 'ASRRules - Get-CDASRRule'
		try
		{
			$Rules = Get-CDASRRule
			if ($null -eq $Rules)
			{ throw 'Get-CDASRRule returned null' }
			$Count = @($Rules).Count
	...
	{
		Write-TestHeader 'ASRExclusions - Get-CDASRExclusion'
		try
		{
			$Excl = Get-CDASRExclusion
	...
	{
		Write-TestHeader 'AllowedApps - Get-CDAllowedApplication'
		try
		{
			$Apps = Get-CDAllowedApplication
	...
	{
		Write-TestHeader 'ControlledFolders - Get-CDControlledFolder'
		try
		{
	...
	{
		Write-TestHeader 'Events - Get-CDEvent'
		try
		{
			$Events = Get-CDEvent
	...
	if ($WriteTestsRequested -and -not $SkipWriteTests)
	{
		...
		if ($Test -contains 'SetASRRule')
		{
			...
			try
			{
				$Rules = Get-CDASRRule | Where-Object { $_.Action -ne 'Not Set' }
				if (-not $Rules)
				...
				$AfterRules = Get-CDASRRule | Where-Object { $_.GUID -eq $Rule.GUID }
