...
$TsBtnThreatRefresh.Add_Click({
		...
		try
		{
			foreach ($T in @(Get-CDThreatAction))
			{
...
$TsBtnEvRefresh.Add_Click({
		...
		try
		{
			...
			$Since     = switch ($SinceKey)
			{
				...
				default        { $null }  # Since Boot - Get-CDEvent default
			}
			...
			[array]$Events = if ($Since) { Get-CDEvent -Filter $FilterVal -Since $Since }
			else         { Get-CDEvent -Filter $FilterVal }
		...
		catch
		{
			...
			try { Write-OperationError -Operation ('Get-CDEvent -Filter {0}' -f $FilterVal) -ErrorInfo $_ } catch { Write-MyCatchAudit -Source 'GUI-Tab-ThreatEvents: Write-OperationError call itself failed' -ErrorRecord $_ }
...
function Get-SacCapture
{
	...
	$Allows = @()
	try { $Allows = @(Get-CDEvent -Filter SAC-Allow) }
	catch { throw ('Get-SacCapture: could not read SAC-Allow events - refusing to save a partial capture. {0}' -f $_.Exception.Message) }
	# Include ALL SAC blocks from the current session (Get-CDEvent default = since last boot), so
	...
	$Blocks = @()
	try { $Blocks = @(Get-CDEvent -Filter SAC) }
...
$TsBtnLogAllows.Add_CheckedChanged({
		...
		if ($TsBtnLogAllows.Checked)
		{
			...
			$LAExisting  = @()
			$LAReadFailed = $false
			try { $LAExisting = @(Get-CDEvent -Filter SAC-Allow) }
